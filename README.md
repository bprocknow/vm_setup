# kernelvm

`kernelvm` is a Python CLI for disposable Fedora KVM/QEMU guests used in Linux kernel testing. Each deployment gets its own run ID, writable qcow2 overlay, cloud-init inputs, serial console artifacts, and host-side metadata so runs can be restarted or destroyed cleanly.

The VM boots with SeaBIOS, which keeps Secure Boot disabled explicitly rather than relying on QEMU firmware defaults.

## Host prerequisites

- Fedora host with `python3`
- `qemu-system-x86_64`
- `qemu-img`
- `cloud-localds`
- `mkfs.ext4` from `e2fsprogs`
- bridge networking already configured on the host, such as `br0`
- SSH public keys for root access
- a Fedora cloud base qcow2 image
- host-built kernel artifacts, at minimum a kernel image and modules archive

## Bridge setup

The repository includes a helper script that creates `br0`, attaches an uplink, updates `/etc/qemu/bridge.conf`, and runs a post-setup sanity test:

```bash
sudo ./br_setup eno1
```

Use `./br_setup --help` to see optional flags such as `--bridge` and `--check-only`.

sudo dnf install qemu-system-x86 bridge-utils
sudo mkdir -p /etc/qemu
echo 'allow br0' | sudo tee /etc/qemu/bridge.conf
sudo chmod 0640 /etc/qemu/bridge.conf

Then create the bridge and move your host NIC onto it. Replace enp1s0 with your actual uplink:

nmcli connection add type bridge ifname br0 con-name br0
nmcli connection add type bridge-slave ifname INTERFACE master br0
nmcli connection modify br0 ipv4.method auto ipv6.method auto
nmcli connection up br0

Before launching a VM, verify that br0 is the active network path instead of the physical NIC:

ip -brief addr show br0
bridge link
ip route show

If br0 is DOWN, has no attached interfaces, or the default route still points at the physical NIC, move the uplink and host IP configuration onto br0 first. Otherwise the guest will boot with only link-local IPv6 and will never receive DHCP.

## Install

  - make venv to create and install the editable package into .venv
  - make test to run the unit tests through tox

```bash
make venv
```
```bash
make test
```

## CLI commands

```bash
kernelvm validate-config path/to/config.yaml
kernelvm tui path/to/config.yaml
kernelvm create path/to/config.yaml
kernelvm list-runs
kernelvm status <run-id>
kernelvm ssh-info <run-id>
kernelvm console <run-id>
kernelvm console <run-id> --attach
kernelvm stop <run-id>
kernelvm start <run-id>
kernelvm destroy <run-id>
```

Use `--work-root /path/to/work` to override the default `./work` run storage directory.
Use `kernelvm tui path/to/config.yaml` to start the interactive VM manager with a create configuration.
Use `kernelvm tui` to manage existing runs without preselecting a create config.

## Run directory layout

Each run is stored under `work/<run-id>/`:

```text
work/<run-id>/
  config/
  logs/
  serial/
  cloud-init/
  overlay/
  artifacts/
  metadata.json
```

Important artifacts:

- `config/input-config.yaml`: original config snapshot
- `config/normalized-config.yaml`: normalized config used by the tool
- `logs/kernelvm.log`: CLI activity log
- `logs/qemu.log`: QEMU process log
- `serial/console.log`: captured serial output
- `cloud-init/`: rendered NoCloud inputs and seed image
- `artifacts/payload/`: staged copy-in files, kernel artifacts, manifest, and first-boot scripts
- `artifacts/payload.img`: generated ext4 payload image attached read-only to the guest for large artifact transfer

## Example config

```yaml
vm_name: kernel-test
base_image_path: /var/lib/vm-images/Fedora-Cloud-Base.qcow2
vcpus: 4
memory_mb: 8192
disk_size_gb: 40
bridge_name: br0
hostname: kernel-test-vm

root_ssh_authorized_keys:
  - ssh-ed25519 AAAA... your-key

packages:
  - gcc
  - make
  - git
  - vim

copy_files:
  - src: /path/on/host/configs
    dest: /root/configs

first_boot_commands:
  - systemctl enable sshd

kernel_artifacts:
  # These stay as absolute host input paths. kernelvm boots kernel_image directly through QEMU
  # and stages only the non-boot artifacts needed during provisioning into the run payload.
  kernel_image: /path/to/bzImage
  kernel_modules_archive: /path/to/modules.tar.zst
  system_map: /path/to/System.map
  config: /path/to/config
  initramfs: /path/to/initramfs.img  # optional, but often needed for Fedora cloud guests

kernel_cmdline_append:
  - root=UUID=<guest-rootfs-uuid>    # required for direct kernel boot
  - console=ttyS0,115200n8           # optional if serial_log_enabled=true; kernelvm adds it if omitted
  - earlycon

qemu_gdb_debug: false
serial_log_enabled: true
```

## Workflow

1. Validate the config and host prerequisites with `kernelvm validate-config`.
2. Create a new run with `kernelvm create`, which creates a fresh overlay, stages the payload directory, builds `artifacts/payload.img`, renders cloud-init artifacts, and launches QEMU with `-kernel` pointing at the configured `kernel_image`.
3. Inspect the deployment with `kernelvm status` and `kernelvm ssh-info`.
   These commands now report guest network readiness, the detected IP source, and any recorded readiness failure reason.
4. Use `kernelvm console --attach` for serial access or inspect `serial/console.log`.
5. Stop and restart an existing run with `kernelvm stop` and `kernelvm start`.
6. Remove the run completely with `kernelvm destroy`.

## Notes

- The tool treats IP discovery as best effort. `ssh-info` and `status` always show hostname and MAC address, and add the guest IP when the host can discover it.
- Bridged guest networking is considered ready only when the tool observes a usable IPv4 address; if that never appears, `status` and `ssh-info` report the recorded readiness failure reason instead of only showing an unknown IP.
- Guest network config is rendered against the launched NIC's MAC address rather than assuming a fixed guest interface name such as `eth0`.
- `create` refuses to start a second run while another run is active.
- The tool does not mutate the base image.
- New runs attach an ext4 payload image for staged files and kernel artifacts; legacy runs that only have the older payload directory metadata still start with the previous compatibility path.
- `qemu_gdb_debug: true` adds `-s -S` to the QEMU launch, so the guest starts paused until a debugger continues execution.
- Kernel artifact config entries such as `kernel_image` are absolute host input paths; any internal staging path is temporary implementation detail, not part of the config contract.
- Direct kernel boot requires a usable `root=` argument in `kernel_cmdline_append`. Use the guest root filesystem UUID or another guest-valid root selector instead of relying on the guest bootloader to provide it.
